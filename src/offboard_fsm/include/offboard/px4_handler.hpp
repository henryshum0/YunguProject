 ...
 *   - in:  vehicle_local_position, vehicle_status, vehicle_land_detected
 *
 ...
 * the getters. Vehicle state (landed / in-air / nav-state / position
 * validity) is derived from the PX4 messages — NOT guessed from z-thresholds.
 */
class Px4Handler
{
public:
    ...
    // --- Derived vehicle state (from PX4 messages, not z-thresholds) ---
    /// True when PX4's landing detector reports the vehicle as landed. Falls
    /// back to dist_bottom (distance to ground) when no land-detected message
    /// has arrived yet.
    bool isLanded() const;
    /// True when the vehicle is airborne (not landed) and the local position
    /// has a valid z estimate.
    bool isInAir() const;
    /// Current PX4 navigation state (VehicleStatus::NAVIGATION_STATE_*).
    uint8_t navState() const;
    /// True when the local position is finite and xy/z are both valid.
...
private:
    ...
    std::shared_ptr<px4_msgs::msg::VehicleLandDetected> land_detected_{nullptr};
};
