...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, PointCloudXYZI::Ptr &pcl_out, const int &lidar_num)
{
  ...
  default:
    // lidar_type 0 (or anything unhandled): generic handler for clouds
    // without ring/time fields (e.g. Gazebo instantaneous scans).
...
void Preprocess::default_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg, const int &lidar_num)
{
  pl_surf.clear();

  // Only x/y/z/intensity are required; ring/time fields are ignored and no
  // per-point deskew is applied (curvature = 0), matching Gazebo's
  // instantaneous scans (the whole frame is treated as rigid).
