...
class SearchMissionSkill(Skill[SearchArea, SearchResult]):
    """Plan a four-corner coverage area, fly it, and report confirmed targets.

    This is the mission-level composite that plans a coverage route, queues it
    with :class:`~skills.skills.navigate.NavigateSkill`, and watches the detector
    with :class:`~skills.skills.detect.DetectSkill` while it flies. It is distinct
    from the command-level :class:`~skills.skills.search.SearchSkill`, which only
    plans and queues without watching for detections.
    """
